...
def test1(dut):
    ...
    yield txData(valid = True, enable = True, value = 0x0F)
    yield txThread.join()
    txThread = cocotb.start_soon(softMaster.sendBitCheck(True, False))
    yield softMaster.wait(10)
    yield txAck(valid=True, enable=True, value=False)
    yield txThread.join()
    ...
    yield softMaster.sendBitCheck(False, False)
    txThread = cocotb.start_soon(softMaster.sendByteCheck(0x22, 0x22))
    ...
    txThread = cocotb.start_soon(softMaster.sendBitCheck(True, True))
    yield softMaster.wait(16)
    yield rxAckValue(False)
    yield txThread.join()
    yield rxAckValue(True)
    ...
    for i in range(2):
        ...
        t = cocotb.start_soon(softMaster.sendBitCheck(True, False))
        yield softMaster.wait(5)
