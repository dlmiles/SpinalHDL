...
class Tester:
    ...
    def driveICmdReady(self):
        dut = self.dut
        dut.io_i_cmd_ready.value = 1
        while True:
            yield [Edge(dut.io_i_rsp_valid),Edge(dut.io_i_rsp_ready)]
            dut.io_i_cmd_ready.value = (int(dut.io_i_rsp_valid) == 0 or int(dut.io_i_rsp_ready) == 1);

    @cocotb.coroutine
    def driveDCmdReady(self):
        dut = self.dut
        dut.io_d_cmd_ready.value = 1
        while True:
            yield [Edge(dut.io_d_rsp_valid),Edge(dut.io_d_rsp_ready)]
            dut.io_d_cmd_ready.value = (int(dut.io_d_rsp_valid) == 0 or int(dut.io_d_rsp_ready) == 1);

    @cocotb.coroutine
    def driveMisc(self):
        self.dut.io_iCmdDrive.value = 1
        self.dut.io_iRspDrive.value = 1
    ...
    def driveIRsp(self):
        ...
        while True:
            ...
            if int(dut.io_i_cmd_valid) == 1 and int(dut.io_i_cmd_ready) == 1:
              dut.io_i_rsp_valid.value = 1
              dut.io_i_rsp_payload_pc.value = int(dut.io_i_cmd_payload_pc)
              data = 0
              if int(dut.io_i_cmd_payload_pc.value) <= 0x03FFFFFF :
                for i in range(0,4):
                  data = data | (self.rom[int(dut.io_i_cmd_payload_pc) + i] << (i*8))

              elif int(dut.io_i_cmd_payload_pc.value) <= 0x04007FFF :
                for i in range(0, 4):
              ...
              if self.wrapICmdZero and int(dut.io_i_cmd_payload_pc) == 0:
                dut.io_i_rsp_payload_instruction.value = 0xffc02e23; #01c02023   ffc02e23
              elif data == 0x00000073:
                dut.io_i_rsp_payload_instruction.value = 0xffc02e23;
              elif data == 0x0FF0000F:
                dut.io_i_rsp_payload_instruction.value = 0x00000013; #TODO remove me
              else:
                dut.io_i_rsp_payload_instruction.value = data;

    @cocotb.coroutine
    def driveDRsp(self):
        logg = open('log.txt', 'wb')
        dut = self.dut
        dut.io_d_rsp_valid.value = 0
        counter = 0
        while True:
            yield RisingEdge(dut.clk)
            counter = counter + 1
            ...
            if int(dut.io_d_cmd_valid) == 1 and  int(dut.io_d_cmd_ready) == 1 :
              if int(dut.io_d_cmd_payload_wr) == 1 :
                ...
                elif int(dut.io_d_cmd_payload_address.value) <= 0x03FFFFFF :
                ...
                elif int(dut.io_d_cmd_payload_address.value) <= 0x04007FFF :
