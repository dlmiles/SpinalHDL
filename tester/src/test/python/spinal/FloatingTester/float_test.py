...
@cocotb.coroutine
def checkZero(dut):
    dut.io_inp_exponent.value = 0
    dut.io_inp_mantissa.value = 0
    yield Timer(1)
...
@cocotb.coroutine
def checkQNaN(dut):
    dut.io_inp_exponent.value = 0xFF
    dut.io_inp_mantissa.value = 0x7FFFFF
    yield Timer(1)
...
def checkSNaN(dut):
    dut.io_inp_exponent.value = 0xFF
    dut.io_inp_mantissa.value = 0x3FFFFF
    yield Timer(1)
    if (int(dut.io_recoded_exp) & 0x1C0) != 0x1C0:
...
def checkUIntToFloat(dut):
    for i in range(100):
        dut.io_in_uint.value = randint(0, 1 << 24 - 1)
        yield Timer(1)
...
def checkSIntToFloat(dut):
    for i in range(100):
        ...
        dut.io_in_sint.value = inputValue
        yield Timer(1)
