...
class SpinalSimBlackboxTester extends SpinalSimFunSuite {
  test("test"){
    SpinalTesterSimConfig(this, "test")
      .addRtl(s"tester/src/test/python/spinal/BlackBoxTester/BlackBoxToTest.${if(tester.language == VHDL) "vhd" else "v"}")
      .doSim(new BlackboxTester.BlackBoxTester) {dut =>
