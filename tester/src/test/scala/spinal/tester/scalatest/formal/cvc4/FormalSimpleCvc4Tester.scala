...
class FormalSimpleCvc4Tester extends SpinalFormalFunSuite {
  ...
  test("Formal_cvc4_Simple_pass") {
    assume(SpinalTesterFormalConfig.pathContainsBinary("cvc4"), "Skipping: cvc4 missing from $PATH")
    startFormalWithBMC(label="Formal_cvc4_Simple_pass")
  }
  test("Formal_cvc4_Simple_fail") {
    assume(SpinalTesterFormalConfig.pathContainsBinary("cvc4"), "Skipping: cvc4 missing from $PATH")
    shouldFail(startFormalWithBMC(1, label="Formal_cvc4_Simple_fail"))
  }
}
